...
export default function Owners() {
  ...
  const addApartment = () => {
    ...
    setApartments(prev => [...prev, { id, name, notes: '', owner: '', ownerPhone: '', ama: '' }]);
    markDirty();
    setNewAptName('');
  };
            ...
            <div key={apt.id} className="border border-slate-100 rounded-lg p-3">
              {editApt?.id === apt.id ? (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-500 mb-1 block">ΑΜΑ Number</label>
                    <input
                      type="text" value={editApt.ama ?? ''}
                      onChange={e => setEditApt(a => ({ ...a, ama: e.target.value }))}
                  ...
                  <div className="flex-1">
                    ...
                    {apt.owner ? (
                      <div className="flex flex-wrap gap-3 mt-1">
                        ...
                        {apt.ama && <span className="text-xs text-slate-400">🔢 ΑΜΑ: {apt.ama}</span>}
                      </div>
                    ) : (
        ...
        {accountantDraft ? (
          <div className="space-y-2">
            <div>
              <label className="text-xs font-medium text-slate-500 mb-1 block">Name</label>
              <input
                type="text" value={accountantDraft.name} autoFocus
                onChange={e => setAccountantDraft(a => ({ ...a, name: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs font-medium text-slate-500 mb-1 block">Phone</label>
                <input
                  type="text" value={accountantDraft.phone ?? ''}
                  onChange={e => setAccountantDraft(a => ({ ...a, phone: fmtPhone(e.target.value) }))}
                  placeholder="+30 694 123 4567"
                  className="w-full border border-slate-200 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  type="email" value={accountantDraft.email ?? ''}
                  onChange={e => setAccountantDraft(a => ({ ...a, email: e.target.value }))}
            ...
            <div className="flex gap-2">
              <button onClick={saveAccountant}
                className="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">Save</button>
              <button onClick={() => setAccountantDraft(null)}
                className="px-3 py-1 border border-slate-200 text-slate-600 rounded text-sm hover:bg-slate-50">Cancel</button>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-3">
            <div className="flex-1 flex flex-wrap items-center gap-3">
              {accountant.name ? (
                <>
                  <span className="text-sm font-medium text-slate-800">👤 {accountant.name}</span>
                  {accountant.phone && <span className="text-xs text-slate-400">📞 {accountant.phone}</span>}
                  {accountant.email && <span className="text-xs text-slate-400">📧 {accountant.email}</span>}
                </>
              ) : (
                <span className="text-sm text-slate-400 italic">No accountant set</span>
              )}
            </div>
            <button onClick={() => setAccountantDraft({ ...accountant })}
              className="text-xs text-slate-400 hover:text-blue-600">Edit</button>
          </div>
        )}
      </div>
