...
// File-only mode helpers: when opening the HTML directly (file://),
// emulate the API in-memory and persist edits to localStorage.
const FILE_MODE = window.location.protocol === "file:";
const LS_PREFIX = "vardania.";
function lsKey(name) {
  return LS_PREFIX + name;
}
function readData(name) {
  if (!FILE_MODE) throw new Error("readData called outside file mode");
  try {
...
function writeData(name, arr) {
  if (!FILE_MODE) throw new Error("writeData called outside file mode");
  try {
    localStorage.setItem(lsKey(name), JSON.stringify(arr));
  } catch (e) {}
}
...
async function apiFetch(path, options = {}) {
  ...
  const get = (n) => readData(n);
  const set = (n, a) => writeData(n, a);
  ...
  try {
    if (method === "GET") {
      if (path === "/api/bookings") return respond(get("bookings"));
      if (path === "/api/expenses") return respond(get("expenses"));
      if (path === "/api/apartments") return respond(get("apartments"));
      if (path === "/api/deleted/bookings")
        return respond(get("deletedBookings"));
      if (path === "/api/deleted/expenses")
        return respond(get("deletedExpenses"));
    }
    if (method === "POST") {
      if (path === "/api/bookings") {
        const arr = get("bookings");
        const nextId =
          arr.reduce((m, b) => Math.max(m, Number(b.id || 0)), 0) + 1;
        const nb = { id: nextId, ...body };
        ...
        arr.push(nb);
        set("bookings", arr);
        return respond(nb, 201);
      }
      if (path === "/api/expenses") {
        const arr = get("expenses");
        const nextId =
          arr.reduce((m, e) => Math.max(m, Number(e.id || 0)), 0) + 1;
        const ne = { id: nextId, ...body };
        ...
        arr.push(ne);
        set("expenses", arr);
        return respond(ne, 201);
      }
      const restoreBookingMatch = path.match(
        /^\/api\/deleted\/bookings\/([^/]+)\/restore$/,
      );
      if (restoreBookingMatch) {
        const id = restoreBookingMatch[1];
        const deleted = get("deletedBookings");
        ...
        set("deletedBookings", deleted);
        const bookings = get("bookings");
        bookings.push(item);
        set("bookings", bookings);
        return respond(item, 200);
      }
      const restoreExpenseMatch = path.match(
        /^\/api\/deleted\/expenses\/([^/]+)\/restore$/,
      );
      if (restoreExpenseMatch) {
        const id = restoreExpenseMatch[1];
        const deleted = get("deletedExpenses");
        const idx = deleted.findIndex((d) => String(d.id) === String(id));
        if (idx === -1) return respond({ error: "Not found" }, 404);
        const item = deleted.splice(idx, 1)[0];
        set("deletedExpenses", deleted);
        const expensesArr = get("expenses");
        expensesArr.push(item);
        set("expenses", expensesArr);
        return respond(item, 200);
      }
    }
    if (method === "PUT") {
      const m = path.match(/^\/api\/bookings\/([^/]+)$/);
      if (m) {
        const id = m[1];
        const arr = get("bookings");
      ...
      const m2 = path.match(/^\/api\/expenses\/([^/]+)$/);
      if (m2) {
        const id = m2[1];
        const arr = get("expenses");
...
// When in file mode, seed localStorage from `server/data/*.json` (if empty)
// and override global `fetch` to route API calls to `apiFetch`.
if (FILE_MODE) {
  const ORIGINAL_FETCH = window.fetch.bind(window);

  (async function seedFromServerData() {
    const fileMap = {
      bookings: "bookings",
      expenses: "expenses",
    ...
    for (const [name, file] of Object.entries(fileMap)) {
      ...
      try {
        ...
        // ignore fetch errors (browsers may block file:// fetches)
      }
    }
  })();
  ...
  window.fetch = async function (url, options = {}) {
    try {
      ...
      if (!u.pathname.startsWith("/api/")) return ORIGINAL_FETCH(url, options);
      return apiFetch(u.pathname, options);
    } catch (e) {
...
function App({ initialBookings = [], initialExpenses = [], initialApts = [] }) {
  ...
  // GitHub sync removed — running with local server only
  useEffect(() => {
    try {
